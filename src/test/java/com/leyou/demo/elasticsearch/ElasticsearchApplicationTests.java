...
public class ElasticsearchApplicationTests {
    ...
    public void patchIndex() {
        Item item1 = new Item(2L, "华为手机8", " 手机", "小米", 3599.00, "http://image.leyou.com/13123.jpg");
        Item item2 = new Item(3L, "小米手机9", " 手机", "华为", 3699.00, "http://image.leyou.com/13123.jpg");
    ...
    public void testSubAgg() {
        ...
        //在brands下添加一个子聚合,聚合类型为avg，聚合名称为avgPrice，聚合字段为price
        searchQueryBuilder.addAggregation(AggregationBuilders.terms("brands").field("brand")
                .subAggregation(AggregationBuilders.avg("avgPrice").field("price")));
        //开始搜索,搜索结果
        //查询 需要把结果强转为AggregatePage 类型
        AggregatedPage<Item> list = (AggregatedPage<Item>) itemMapper.search(searchQueryBuilder.build());
        //解析
        //从结果中取出brands的那个聚合
        StringTerms agg = (StringTerms) list.getAggregation("brands");
        //获取brand下的桶
        List<StringTerms.Bucket> buckets = agg.getBuckets();
        buckets.forEach(s -> {
            //获取桶中的key，即品牌名称 获取桶中的数量
            System.out.println(s.getKeyAsString() + "---" + s.getDocCount());
            //获取子聚合结果
            InternalAvg avg = (InternalAvg) s.getAggregations().asMap().get("avgPrice");
